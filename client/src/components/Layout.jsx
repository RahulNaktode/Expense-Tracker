...
import { 
    Utensils, Home, Car, ShoppingCart, Gift, Zap, Activity, ArrowUp, 
    CreditCard, ArrowDown, Clock, DollarSign, PiggyBank, RefreshCw, 
    TrendingUp, Info, ChevronDown, ChevronUp, PieChart 
} from 'lucide-react';
...
function Layout({ onLogout, user }) {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            const [incomeRes, expenseRes] = await Promise.all([
                axios.get(`http://localhost:8000/income/get`, { headers }),
                axios.get(`http://localhost:8000/expense/get`, { headers }),
            ]);
            ...
            const allTransactions = [...incomes, ...expenses]
                .map((t) => ({
                    ...
                    description: t.description || t.title || t.note || "No description",
    ...
    const addTransaction = async (transaction) => {
        try {
            ...
            await axios.post(`http://localhost:8000/${endpoint}`, transaction, { headers });
    ...
    const editTransaction = async (id, transaction) => {
        try {
            ...
            await axios.put(`http://localhost:8000/${endpoint}/${id}`, transaction, { headers });
            await fetchTransactions();
            return true;
        } catch (err) {
    ...
    const deleteTransaction = async (id, type) => {
        try {
            ...
            await axios.delete(`http://localhost:8000/${endpoint}/${id}`, { headers });
            await fetchTransactions();
            return true;
        } catch (err) {
            ...
            <div className={styles.layout.mainContainer(sidebarCollapsed)}>
                ...
                <div className={styles.statCards.grid}>
                    <div className={styles.statCards.card}>
                        ...
                        <p className={styles.statCards.cardFooter}>
                            <span className='text-teal-600 font-medium'>
                                {stats.last30DaysSavings >= 0 ? "+" : ""}${stats.last30DaysSavings.toLocaleString()}
                            </span>{" "}this month
                        </p>
                    </div>
                    ...
                    <div className={styles.statCards.card}>
                        <div className={styles.statCards.cardHeader}>
                            ...
                            <div className={styles.statCards.iconContainer("green")}>
                                <ArrowDown className={styles.statCards.icon("green")} />
                            </div>
                        </div>
                        <p className={styles.statCards.cardFooter}>
                    ...
                    <div className={styles.statCards.card}>
                        <div className={styles.statCards.cardHeader}>
                            <div>
                                <h2 className={styles.statCards.cardTitle}>Savings Rate</h2>
                                <p className={styles.statCards.cardValue}>
                                    {stats.savingsRate}%
                                </p>
                            </div>
                ...
                <div className={styles.grid.main}>
                    ...
                    {/* Transaction List and Category Split Blocks */}
                    <div className={styles.grid.rightColumn}>
                            ...
                            <div className={styles.transactions.listContainer}>
                                ...
                                {transactions.length === 0 ? (
                                    ...
                                    <div className={styles.transactions.viewAllContainer}>
                                        <button className={styles.transactions.viewAllButton} onClick={() => setShowAllTransactions(!showAllTransactions)}>
                                            {showAllTransactions ? (
                                                <>
                        ...
                        <div className={styles.cards.base}>
                            <h3 className={styles.categories.title}>
                                <PieChart className={styles.categories.titleIcon} />Spending Categories
                            </h3>
