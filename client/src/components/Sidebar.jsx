...
import { useLocation, useNavigate, Link } from 'react-router'; // Consolidated imports
...
import { AnimatePresence, motion } from "framer-motion"; // Removed non-existent 'scale' import
...
function Sidebar({ user, isCollapsed, setIsCollapsed }) {
    const { pathname } = useLocation(); // FIXED: changed pathName to pathname
    ...
    // FIXED: Object fallback structure to safely destructure properties
    ...
    const renderMenuItem = ({ text, path, icon }, isMobile = false) => {
        ...
        <>
            ...
            >
                <div className={sidebarStyles.sidebarInner.base}>
                    <button onClick={toggleSidebar} className={sidebarStyles.toggleButton.base} aria-label="Toggle Sidebar">
                        <motion.div
                            initial={{ rotate: 0 }}
                    ...
                    <div className='flex-1 overflow-y-auto py-4 custom-scrollbar'>
                        <ul className={sidebarStyles.menuList.base}>
                            {MENU_ITEMS.map((item) => renderMenuItem(item, false))}
                        </ul>
                    </div>
                        ...
                        >
                            <div className='relative h-full flex flex-col'>
                                <div className={sidebarStyles.mobileHeader}>
                                    ...
                                    <button onClick={() => setMobileOpen(false)} className={sidebarStyles.mobileCloseButton} aria-label="Close Menu">
                                        <X size={24} className='text-gray-600' />
                                    </button>
                                </div>

                                {/* FIXED: Removed opacity-40 completely from layout wrapper */}
