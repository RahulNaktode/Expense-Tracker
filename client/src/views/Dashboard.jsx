...
import { GAUGE_COLORS, COLORS, INCOME_COLORS, EXPENSE_CATEGORY_ICONS, INCOME_CATEGORY_ICONS } from '../assets/color.jsx'
import axios from 'axios'
import { ArrowDown, BarChart2, Cat, ChevronDown, ChevronUp, DollarSign, PieChart as PieChartIcon, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp as ProfitIcon, TrendingUp, Wallet } from 'lucide-react'
...
function Dashboard() {
  ...
  // ✅ Pehle server data check karo (kisi bhi timeFrame ke liye)
  if (
  ...
  // ✅ Fallback — local transactions se calculate karo
  const categories = {};
...
}, [filteredTransactions, overviewMeta]); // ✅ timeFrame dependency hatayi
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      if (newTransaction.type === "income") {
        await axios.post("http://localhost:8000/income/add", payload, {
          headers: getAuthHeader(),
        })
      } else {
    ...
    <div className={dashboardStyles.container}>
      <div className={dashboardStyles.headerContainer}>
        <div className={dashboardStyles.headerContent}>
          <div>
            <h2 className={dashboardStyles.headerTitle}>Finance Dashboard</h2>
            <p className={dashboardStyles.headerSubtitle}>Tracks your income and expense</p>
          </div>
      ...
      <div className={dashboardStyles.summaryGrid}>
        ...
        <FinancialCard icon={
          ...
          additionalContent={
            <div className={`mt-2 text-sm flex items-center gap-1 ${expenseChange >= 0 ? trendStyles.positive : trendStyles.negative
              }`}>
              {expenseChange >= 0 ? (
              ...
              <span>
                ...
                {expenseChange >= 0 ? "increace" : "decrease"} from{" "}
